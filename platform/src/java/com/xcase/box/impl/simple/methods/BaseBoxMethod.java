...
public class BaseBoxMethod {
     ...
     * the configuration.
     */
    protected Properties config = BoxConfigurationManager.getConfigurationManager().getConfig();
...
//        LOGGER.debug("Starting BaseBoxMethod()");
        this.apiOAuthTokenPrefix = config.getProperty(BoxConstant.CONFIG_API_OAUTH_TOKEN_PREFIX);
        //LOGGER.debug("apiOAuthTokenPrefix is " + apiOAuthTokenPrefix);
        this.apiOAuthAuthorizePrefix = config.getProperty(BoxConstant.CONFIG_API_OAUTH_AUTHORIZE_PREFIX);
        //LOGGER.debug("apiOAuthAuthorizePrefix is " + apiOAuthAuthorizePrefix);
        this.apiOAuthRevokePrefix = config.getProperty(BoxConstant.CONFIG_API_OAUTH_REVOKE_PREFIX);
        //LOGGER.debug("apiOAuthAuthorizePrefix is " + apiOAuthAuthorizePrefix);
    ...
    /**
     * According to action name, return a string buffer. i.e. "get_ticket" can
     * result a "http://upload.box.net/2.0/"
     *
