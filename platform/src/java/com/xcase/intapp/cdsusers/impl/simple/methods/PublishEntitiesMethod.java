...
public class PublishEntitiesMethod extends BaseCDSUsersMethod {
	...
	public PublishEntitiesResponse publishEntities(PublishEntitiesRequest request) {
        LOGGER.debug("starting publishEntities()");
        PublishEntitiesResponse response = CDSUsersResponseFactory.createPublishEntitiesResponse();
        ...
        try {
            ...
            endPoint = endPoint.replace("{topic}",  topic).replace("{entity}", entity);
            ...
            CommonHttpResponse commonHttpResponse = httpManager.doCommonHttpResponsePost(endPoint, headers, parameters, null, null);
