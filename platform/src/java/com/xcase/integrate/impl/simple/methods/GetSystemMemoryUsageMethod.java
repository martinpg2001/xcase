 ...
 * @author martinpg
 */
public class GetSystemMemoryUsageMethod extends BaseIntegrateMethod {
    ...
    public GetSystemMemoryUsageResponse getSystemMemoryUsage(GetSystemMemoryUsageRequest request) {
        LOGGER.debug("starting getSystemMemoryUsage()");
        GetSystemMemoryUsageResponse response = IntegrateResponseFactory.createGetSystemMemoryUsageResponse();
        ...
        try {
            ...
            LOGGER.debug("memory is " + memory);
            endPoint = baseVersionUrl + CommonConstant.SLASH_STRING + object + CommonConstant.SLASH_STRING + memory;
...
//            LOGGER.debug("created xHTTPMethodHeader header");
            ...
            if (responseCode == 200) {
                ...
                if (IntegrateConstant.CONFIG_API_REQUEST_FORMAT_JSON.equals(this.apiRequestFormat)) {
                    ...
                    SystemMemoryUsage systemMemoryUsage = gson.fromJson(datasourceJsonObject, SystemMemoryUsage.class);
                    LOGGER.debug("created systemDiskUsage");
                    response.setSystemMemoryUsage(systemMemoryUsage);
                } else if (IntegrateConstant.CONFIG_API_REQUEST_FORMAT_XML.equals(this.apiRequestFormat)) {
                    JAXBContext jaxbContext = JAXBContext.newInstance(SystemMemoryUsage.class);
                    Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
                    StringReader stringReader = new StringReader(responseEntityString);
                    LOGGER.debug("created stringReader");
                    SystemMemoryUsage systemMemoryUsage = (SystemMemoryUsage) jaxbUnmarshaller.unmarshal(stringReader);
                    LOGGER.debug("created systemDiskUsage");
                    response.setSystemMemoryUsage(systemMemoryUsage);
                } else {
