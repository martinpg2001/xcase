...
public class Sharepoint2010FileImpl implements SharepointFile {
    ...
    public void populateFromXmlNode(org.dom4j.Node fileNode) {
        ...
        org.dom4j.XPath contentSourceXPath = fileNode.createXPath("Atom:content/@src");
        contentSourceXPath.setNamespaceURIs(uris);
        LOGGER.debug("set name space URIs for contentSourceXPath");
        org.dom4j.Node entryContentNode = contentSourceXPath.selectSingleNode(fileNode);
        LOGGER.debug("next entryContentNode text is " + entryContentNode.getText());
        /* Got entry content node */
        org.dom4j.XPath propertiesXPath = fileNode.createXPath("m:properties");
        propertiesXPath.setNamespaceURIs(uris);
        LOGGER.debug("set name space URIs for propertiesXPath");
        org.dom4j.Node entryPropertiesNode = propertiesXPath.selectSingleNode(fileNode);
